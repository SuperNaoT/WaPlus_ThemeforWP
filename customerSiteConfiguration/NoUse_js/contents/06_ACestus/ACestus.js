  ...
  window.onscroll = function(e) {
    ...
    if ( document.querySelector( 'body' ).classList.contains( 'MOBILE' ) )  {
      ...
      gsap.to( '#cthCopy1', {
        ...
        // },
        scrollTrigger: {
          trigger: '#ACestusGallery',//アニメーションが始まるトリガーとなる要素
...
function dispEachSubPage() {
  ...
  // ★ サイトメイン画面の表示
  // *********************************************************************
  // document.querySelector( '.ACestusDetailContents' ).classList.add( 'active' );
...
function calcRollingSecHeight() {
  ...
  // console.log( "\n\n 【ACestus】>>>>> Start calcRollingSecHeight() >>>>> " );
  ...
  iWk1 = vhTopx( 30 ) + document.getElementById( "ACestusLgLogoImage" ).clientHeight;

  // secPos = document.getElementById( "ACestusLgLogoImage" ).getBoundingClientRect().top;
  // iWk1   = document.getElementById( "ACestusLgLogoImage" ).clientHeight + secPos;

  // console.log( `    ? This Section's info  TEXT[ ${Math.round(iWrk) + "px"} ]`);
  // console.log( `    ? This Section's info IMAGE[ HEIGHT ${document.getElementById( "ACestusLgLogoImage" ).clientHeight} / BOTTOM ${Math.round(iWk1)+"px"} ]\n`);

  // 
  // 文字サイドのボトム位置と、補完画像のボトム位置を比較し、
  // 低い方を、セクション高の計算値に採用する。
  if ( iWk1 > iWrk )  { iWrk = iWk1; }
                        iWrk = Math.round( iWrk ) + ( topBottomSpace / 2 );
                        iWrk = iWrk - 100;
  ...
  // console.log( " 【ACestus】<<<<< Exit  calcRollingSecHeight() <<<<< \n\n" );
