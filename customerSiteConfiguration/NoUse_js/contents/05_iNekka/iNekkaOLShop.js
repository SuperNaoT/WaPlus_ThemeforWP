  ...
  window.onscroll = function(e) {
    ...
    if ( document.querySelector( 'body' ).classList.contains( 'MOBILE' ) )  {
      ...
      gsap.to( '#cthCopy1', {
        ...
        // },
        scrollTrigger: {
          trigger: '#snsInformation',//アニメーションが始まるトリガーとなる要素
    ...
    gsap.to( '#twitterLogo2', {
      ...
      scrollTrigger: {
        trigger: '#snsInformation',//アニメーションが始まるトリガーとなる要素
        start: '60% 80%', //アニメーションが始まる位置
        end:   '60% 80%', //アニメーションが始まる位置
        // scrub: true,
        // markers: true,
      },
    })
...
function calcRollingSecHeight() {
  ...
  // console.log( "\n\n 【いーねっかオンライン販売】>>>>> Start calcRollingSecHeight() >>>>> " );
  ...
  // console.log( " 【いーねっかオンライン販売】<<<<< Exit  calcRollingSecHeight() <<<<< \n\n" );
...
function calcEachSecHeight( secNo ) {
  ...
  const secElms = [
    {
      // セクション名（ページトップからの位置取得）
      sName  : ["snsInformation"],
      // 事業種別
      cthCpy : ["cthCopy1"],
      // 事業説明詳細文章
      dtlMsg : ["forExample1", "forExample2", "forExample3"],
    },
    // {
    //   // セクション名（ページトップからの位置取得）
    //   sName  : ["ACestusDtlExp"],
  ...
  // console.log( `》「${secElms[secNo]["dtlMsg"][0]}」 SNS-１　高さ【${dMsgHgt[0]}】` );
  
  // ＳＮＳ種別情報２
  dMsgHgt[1] = document.getElementById( secElms[secNo]["dtlMsg"][1] ).clientHeight;
  // console.log( `》「${secElms[secNo]["dtlMsg"][1]}」 SNS-２　高さ【${dMsgHgt[1]}】` );
  
  // ＳＮＳ種別情報３
  dMsgHgt[2] = document.getElementById( secElms[secNo]["dtlMsg"][2] ).clientHeight;
  // console.log( `》「${secElms[secNo]["dtlMsg"][1]}」 SNS-３　高さ【${dMsgHgt[1]}】` );
  
  iWrk = topBottomSpace + cCpyHgt    + intervalSpace
                        + dMsgHgt[0] + intervalSpace
                        + dMsgHgt[1] + intervalSpace
                        + dMsgHgt[2] + topBottomSpace + ulineHgt * 2;

  if ( document.querySelector( 'body' ).classList.contains( 'MOBILE' ) )  {
    //スマホ処理
    // iWrk = iWrk - topBottomSpace - 75;
    iWrk = iWrk + topBottomSpace + 50;
  } else  {
    //PC処理
    iWrk = iWrk - 95;        // -215
  }
