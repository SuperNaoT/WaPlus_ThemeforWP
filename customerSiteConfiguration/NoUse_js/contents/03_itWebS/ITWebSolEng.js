...
document.addEventListener( 'DOMContentLoaded', function() {
  ...
  window.onscroll = function(e) {
    ...
    gsap.to( '#forExample1', {
      ...
      scrollTrigger: {
        trigger: '#rollingTitlePage',//アニメーションが始まるトリガーとなる要素
        start: '50% center', // 60% アニメーションが始まる位置
        end:   '50% center', // 60% アニメーションが始まる位置
        // scrub: true,
        // markers: true,
      },
    })
    gsap.to( '#ITWebEngTopImg2', {
      x: 0,
      y: 0,
      opacity: 1,
      duration: 4.0,  // 1.8 (like2.5)
    ...
    gsap.to( '#forExample2', {
      ...
      scrollTrigger: {
        trigger: '#rollingTitlePage',//アニメーションが始まるトリガーとなる要素
        start: '58% center', //アニメーションが始まる位置
        end:   '58% center', //アニメーションが始まる位置
        // scrub: true,
        // markers: true,
      },
    })
    gsap.to( '#ITWebEngTopImg3', {
      x: 0,
      y: 0,
      opacity: 1,
      duration: 4.0,  // 1.8 (like2.5)
...
function calcSecHgtfromEachElmPos( secNo ) {
  ...
  const secElms = [
    {
      ...
      // 事業説明詳細文章
      dtlMsgCnt: 3,
      dtlMsg : ["forExample", "forExample1", "forExample2"],
      // dtlMsg : ["detailInfo"],
    }
  ]
  ...
  if ( secNo == 0 )  {
    ...
    iWk1 = vhTopx( 150 ) + document.getElementById( "ITWebEngTopImg2" ).clientHeight;

    // console.log( `    ? This Section's info  TEXT[ ${Math.round(iWrk) + "px"} ]`);
    // console.log( `    ? This Section's info IMAGE[ HEIGHT ${document.getElementById( "ITWebEngTopImg2" ).clientHeight} / BOTTOM ${Math.round(iWk1)+"px"} ]\n`);
