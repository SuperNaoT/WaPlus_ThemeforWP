  ...
  window.onscroll = function(e) {
    ...
    if ( document.querySelector( 'body' ).classList.contains( 'MOBILE' ) )  {
      ...
      gsap.to( '#forExample1', {
        ...
        duration: 4.0,  // 1.8 (like2.5)
        scrollTrigger: {
          trigger: '#forExample',//アニメーションが始まるトリガーとなる要素
          start: '90% center', //アニメーションが始まる位置
          end:   '90% center', //アニメーションが始まる位置
          // scrub: true,
          // markers: true,
        },
      })
...
function dispEachSubPage() {
  ...
  // ★ サイトメイン画面の表示
  // *********************************************************************
  document.querySelector( '.privacyPolicyContents' ).classList.add( 'active' );
...
function calcSecHgtfromEachElmPos( secNo ) {
  ...
  // console.log( `\n\n 【プライバシーポリシー】>>>>> Start calcSecHgtfromEachElmPos( ${secNo} ) >>>>> ` );
  ...
  // console.log( ` 【プライバシーポリシー】<<<<< Exit  calcSecHgtfromEachElmPos( ${secNo} ) <<<<< \n\n` );
