  ...
  <?php
    ...
    // echo "<br> >>>>> Enter WaPlusThemeCreateNavMenu.php >>>>> <br>";
    ...
    for( $i=0 ; $i< $GB_maxNavMenuCount ; $i++ ) { $menuColor[$i]  = 'color: ' . $menuText_color . ';'; }
                                          $menuColor[$args['GB_pageKind']] = 'color:limegreen !important;';
                                          $pageLink [$args['GB_pageKind']] = '#mainArea';

    // echo "<br> >>>>> Enter WaPlusThemeCreateNavMenu.php >>>>> \$GB_pageKind【" . $args['GB_pageKind'] . "】 <br>";
    ...
    if ( ( $locations = get_nav_menu_locations() ) && isset( $locations[ $menu_name ] ) ) {
        ...
        foreach ( (array) $menu as $key => $menu_item ) {
          ...
          if ( !$menu_item->menu_item_parent )  {
            ...
            if ( $GB_page_flag != 1 )  {
              $menu_list .= '<li id="' . $curIDName . '">';
              $menu_list .= '<a style="' . $menuColor[$count] . '" href="' . $url . '?loadScreen=no">' . $title . '</a></li>';              
            //
          ...
          if ( $parent_id == $menu_item->menu_item_parent )  {
            ...
            // if ( $chrChk == '</li>' && $subMenuWithParent )  {
            if ( $chrChk == '</li>' && !$submenu )  {
                 $chrChk = substr( $menu_list, 0, (strlen($menu_list)-31 ) ); // '</li>' + '<li class="navInter"></li>'
              $menu_list = $chrChk;
            }
            ...
            if ( !$submenu )  {
              $menu_list .= '<div class="' . $curIDName . '_subMenu"' . ' id="' . $curIDName . '_subMenu">';
              $submenu    = true;
              $menu_list .= '<ul>';
            }
            ...
            // ■ 表示ページが、お問い合わせ入力・確認ページ以外の場合
            if ( $GB_page_flag != 1 )  {
              $menu_list .= '<li><a style="' . $menuColor[$count] . '" href="' . $submenu_url . '">' . $submenu_title . '</a></li>';
    ...
    // echo " <<<<< Exit- WaPlusThemeNavCreateMenu.php <<<<< <br>";                                       
